...
const OracleCaller = (options: OracleCallerOptions) => {
  ...
  const injectionStringPresent = !_url.includes(POPAYLOAD) && !String(typeof headers === 'object' ? JSON.stringify(headers) : headers).includes(POPAYLOAD) && !(data || '').includes(POPAYLOAD);
