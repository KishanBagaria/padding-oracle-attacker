...
const PaddingOracle = (options: PaddingOracleOptions) => {
  ...
  const {
    ...
    logMode = 'full',
    isCacheEnabled = true,
    initFirstPayloadBlockWithOrigBytes = false,
  } = options;
  ...
  async function processByte({ blockI, byteI, byte, currentPadding, offset }: { blockI: number; byteI: number; byte: number; currentPadding: number; offset: number }): Promise<boolean> {
    ...
    if (logMode === 'full' && !stopLoggingProgress) {
      if (!(foundOffsets.has(offset) && !decryptionSuccess)) {
        // make sure concurrency doesn't cause former bytes progress to be logged after later byte
  ...
  async function processBlock(blockI: number) {
    ...
    for (const byteI of range(blockSize - 1, -1)) {
      ...
      const byteRange = isDecrypting ? range(0, 256).filter((b) => b !== cipherByte) : range(0, 256);
      if (concurrency > 1) {
